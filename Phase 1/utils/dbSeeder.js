// New database seeder utility to populate initial data
...
const seedDatabase = async () => {
  try {
    ...
    // Check each table separately and seed only if necessary
    const userCount = await User.count();
    const bookCount = await Book.count();
    const loanCount = await Loan.count();
    
    // We'll seed tables individually instead of skipping entirely
    console.log(`Current database state: ${userCount} users, ${bookCount} books, ${loanCount} loans`);
    ...
    // Seed some initial loans if needed
    if (loanCount === 0 && userCount > 0 && bookCount > 0) {
      // Get user and book IDs for creating loans
      const users = await User.findAll();
      const books = await Book.findAll();
      
      if (users.length >= 2 && books.length >= 2) {
